...
export const SettingsScreen: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    setSettings(updated);
    setSaving(false);
    Alert.alert('Settings Saved', 'Your schedule, gym workout & Sleep Force timings have been updated.');
  };
  ...
  const handleSyncLocation = async () => {
    ...
    if (coords) {
      ...
      await StorageService.saveSettings(updated);
      setSettings(updated);
      Alert.alert('Location Updated', `GPS location updated to: ${coords.city}`);
    } else {
      Alert.alert('Location Error', 'Unable to fetch GPS coordinates. Please check location permissions.');
    }
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        ...
        <Text style={styles.subtitle}>Customize office, gym & sleep force routine</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        ...
        <GlassCard accentColor="#3B82F6">
          ...
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Commute Duration (Minutes each way)</Text>
            <TextInput
              style={styles.input}
        ...
        <GlassCard accentColor="#EF4444" style={{ marginTop: 12 }}>
          ...
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Morning Gym Start Time (24h format)</Text>
            <TextInput
              style={styles.input}
              value={gymStart}
          ...
          <View style={styles.inputGroup}>
            <Text style={styles.label}>Sleep Force Routine Start Time (24h format)</Text>
            <TextInput
              style={styles.input}
              value={sleepStart}
          ...
          <TouchableOpacity style={styles.saveBtn} onPress={handleSave} disabled={saving}>
            <Text style={styles.saveBtnText}>{saving ? 'Saving...' : 'Save All Schedule Settings'}</Text>
          </TouchableOpacity>
        </GlassCard>
        ...
        <GlassCard accentColor="#10B981" style={{ marginTop: 12 }}>
          ...
          <Text style={styles.infoText}>
            Current Location: <Text style={{ color: '#10B981', fontWeight: '700' }}>{settings?.cityName || 'Karachi'}</Text>
          </Text>
          <Text style={styles.infoSubtext}>
            Lat: {settings?.latitude?.toFixed(4)}, Long: {settings?.longitude?.toFixed(4)}
          </Text>
        ...
        <GlassCard accentColor="#8B5CF6" style={{ marginTop: 12 }}>
          ...
          <Text style={styles.privacyText}>
            All daily planner data, prayer caches, 503 lesson progress, and task checklists are stored directly on your phone via local AsyncStorage. No cloud accounts, tracking, or external server dependencies.
          </Text>
        </GlassCard>
      </ScrollView>
    </View>
