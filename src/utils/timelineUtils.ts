import { PrayerTimings, UserSettings, TimelineItem, PrayerName, DailyPrayerCheck } from '../types';
...
export const generateDailyTimeline = (
  ...
  items.push({
    ...
    subTitle: 'Physical fitness & shower • 1 hr 15 min',
  ...
  const eveningCommuteEndMins = Math.min(24 * 60 - 1, officeStartMins + 480 + commuteMins);
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const nextItemMins = i < items.length - 1 ? timeToMinutes(items[i + 1].startTime) : 24 * 60;

    if (currentNowMins < itemMins) {
      items[i].isPast = false;
    } else if (currentNowMins >= itemMins && currentNowMins < nextItemMins && !activeFound) {
